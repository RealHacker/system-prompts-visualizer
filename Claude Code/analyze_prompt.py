...
def extract_sections_and_content(file_path):
    ...
    for line in content.split('\n'):
        ...
        if line.strip().startswith('<') and '>' in line.strip() and not line.strip().startswith('</'):
            ...
            current_text = ""
        elif line.strip().startswith('IMPORTANT:') and current_section == "Introduction":
            # Save previous section when we hit IMPORTANT statements
            if current_text.strip():
                sections[current_section] = current_text.strip()
            current_section = "Important Guidelines"
            current_text = line + "\n"
        else:
            current_text += line + "\n"
...
def extract_role_and_identity(content):
    ...
    role_patterns = [
        r'You are.*?(?:an AI|AI assistant|agent|assistant|interactive CLI tool)',
        r'Role.*?:.*?(?:an AI|AI assistant|agent|assistant|interactive CLI tool)',
        r'Identity.*?:.*?(?:an AI|AI assistant|agent|assistant|interactive CLI tool)'
    ]
...
def extract_workflow_steps(content):
    ...
    # Look for step-by-step instructions or workflows
    step_patterns = [
        r'-\s*(.+)',
        r'\d+\.\s*(.+)'
    ]
    ...
    for line in lines:
        for pattern in step_patterns:
            ...
            if match:
                ...
                if len(step) > 10 and len(step) < 200:  # Reasonable step length
...
def extract_rules(content):
    ...
    # Look for rules (often in format "IMPORTANT:", "You MUST", etc.)
    rule_indicators = [
        r'IMPORTANT:\s*(.+)',
        r'You\s+(MUST|SHOULD|NEVER)\s+(.+)',
        r'(MUST|SHOULD|NEVER)\s+(.+)',
        r'When\s+(.+),\s+(.+)'  # When X, do Y pattern
    ]
